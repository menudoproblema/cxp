...
SUPPORTED_PROTOCOL_VERSIONS: tuple[ProtocolVersion, ...] = (CURRENT_PROTOCOL_VERSION,)
...
def is_protocol_version_supported(
    ...
    supported_protocol_versions: tuple[
        ProtocolVersion, ...
    ] = SUPPORTED_PROTOCOL_VERSIONS,  # noqa: E501
) -> bool:
...
def negotiate_capabilities(
    ...
    if negotiated_protocol_version is None:
        supported_versions_rendered = (
            ", ".join(str(version) for version in supported_protocol_versions) or "none"
        )
    ...
    missing_optional = [
        name
        for name in request.optional_capabilities
        if not available_capabilities.has_capability(name)
    ]
    ...
    if missing_optional:
        return HandshakeResponse(
            ...
            reason=("Missing optional capabilities: " + ", ".join(missing_optional)),
            missing_required_capabilities=(),
